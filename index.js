		...
		case 'log':
			{
				...
				function askForServingSize () {
					readline.question(
						`How many servings did you eat? Enter decimal: 1, 0.5, 1.25, etc...--> `,
						servingSize => {
				...
				async function displayCalories (servingSize = 1, food) {
					...
					console.log(`${food.name} with a serving size of ${servingSize} has a 
				${Number.parseFloat(
				...
				readline.question('What would you like to log today? ', async item => {
					...
					while (!position.done) {
						...
						if (food === item) {
							console.log(`${item} has ${position.value.calories} kCal!`);
			...
			readline.question('Email: ', async emailAddress => {
				...
				for (const entry of logIterator) {
					...
					if (isToday(new Date(Number(timestamp)))) {
						console.log(
							`${entry[timestamp].food}, ${entry[timestamp]
								.servingSize} serving(s)`
						);
						totalCalories += entry[timestamp].calories;
						if (totalCalories >= 12000) {
							console.log(`Impressive! You've reached 12,000 kCals!!!`);
							logIterator.return();
						}
					}
				}
